    ...
    <div class="w-[1200px]">
      ...
      <div @click="goDetail(item)" v-for="item in list" class="border-b border-dashed border-[#ddd] flex">
        <div class="w-full flex justify-between leading-[50px]">
          <div class="w-[1000px] text-[16px] cursor-pointer overflow-hidden whitespace-nowrap text-ellipsis dot">问题{{ item.index }}：<span>{{ item.title }}</span></div>
          <div class="text-[12px] text-[#999]">发布时间：{{ item.gmtCreate }}</div>
        </div>
      </div>
...
const getList = (pageNum:number) => {
  ...
  getQuestionListAll(params).then((res:any) => {
    loading.value = false
    if (res.code === 200) {
      res.rows.forEach((item:any, i:any) => item.index = i+1)
      list.value = res.rows
...
const goDetail = (row:any) => {
  router.push({ name: 'QandADetail', query: { headline: row.title } })
}
</script>
